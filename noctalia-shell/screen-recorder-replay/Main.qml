...
Item {
    ...
    property bool savedReplay: false
    ...
    function launchRecorder() {
        ...
        if (replayLength === "0") {
            ...
            var flags = `-w ${videoSource} -f ${frameRate} -k ${videoCodec} ${audioFlags} -q ${quality} -cursor ${showCursor ? "yes" : "no"} -cr ${colorRange} ${resolutionFlag} -o "${videoDir}" -c mp4 -r ${replayLength}`
        }
        var command = `
            ...
            if (isPending) {
                ...
                if (exitCode === 0 || savedReplay) {
                    ...
                    if (!wasCancelled) {
                        if (stderr.length > 0) {
                            //ToastService.showError(pluginApi.tr("messages.failed-start"), truncateForToast(stderr))
                            ToastService.showError(pluginApi.tr("messages.failed-start"), "Inside !wasCancelled")
                            Logger.e("ScreenRecorder", stderr)
                        } else {
